...
class _ms(object):
    REGEX_UID = re.compile(ur" using unique (\w*)", re.I)
    REGEX_SRID = re.compile(ur" using srid=([-]?\d*)", re.I)
    REGEX_FROMSOURCE = re.compile(ur" from ", re.I)
    REGEX_SPACE = re.compile(ur" ") #\s+
    REGEX_TABLE = re.compile(ur"(?P<schema>.*)\.(?P<table>.*)|(?P<table_>.*)")
    REGEX_SUBSELECT = re.compile(ur"(\(.*\)) as", re.I | re.S | re.M | re.U)
    REGEX_SOURCE_POSTGIS = re.compile(ur"(\(.*\) as \w*|\"?\w*\"?\.\"?\w*\"?|\"?\w*\"?)", re.I | re.S | re.M | re.U)
    REGEX_NATIVE = re.compile(ur"^NATIVE_FILTER=(.*)$")
    REGEX_SUBSTITUTION = re.compile(ur"(%+\w*%+)+")
    REGEX_PATTERN = re.compile(ur"^\\((.*?)\\)$")
    REGEX_STRING = re.compile(ur"^(\w*)$")
    REGEX_REGEX = re.compile(ur"^/(.*)/$") #ur"^/(.*?)/$"
    REGEX_PROJECTION = re.compile(ur"(epsg:\d*)$", re.I)
...
class _qgis(object):
    REGEX_ATTR = re.compile(ur'^\[(.*)\]$') #^\[(\w*)\]$'
    REGEX_URL = re.compile(ur'^http', re.I)
    REGEX_FILE = re.compile(ur"^(?!http://).*\.([a-z]{3,4})$", re.I)
    REGEX_CHAR = re.compile(ur"&#(.*);")
