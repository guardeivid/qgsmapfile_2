...
REGEX_STRING = re.compile(ur"^(\w*)$")
REGEX_REGEX = re.compile(ur"^/(.*)/$")
REGEX_LIST = re.compile(ur"^{(.*)}$")
REGEX_LOGICAL = re.compile(ur"(\(.*\))$")
REGEX_BETWEEN = re.compile(ur"\[(?P<attr1>\w*)\] >=? (?P<min>\d*\.?\d*) AND \[(?P<attr2>\w*)\] <=? (?P<max>\d*\.?\d*)", re.I)
REGEX_CONCAT_ATTR = re.compile(ur"((\[\w*\])(?:\s*)){2,}")
REGEX_CONCAT_STR = re.compile(ur"^['|\"]+.*['|\"]+$|tostring\(.*")
REGEX_FX_TOSTRING = re.compile(ur"tostring(\(\[\w*\]),\s?[\'|\"]{1}\%\.(\d)f[\'|\"]{1}\)")
#REGEX_FX_UPPER = re.compile(ur"upper\('?\"?\[\w*\]'?\"?\)")
#REGEX_FX_LOWER = re.compile(ur"lower\('?\"?\[\w*\]'?\"?\)")
REGEX_FX_INITCAP = re.compile(ur"(initcap|firscap)(\('?\"?\[\w*\]'?\"?\))") #solo fx([atribute])?
REGEX_LOGICAL_IN = re.compile(ur"'?\"?(\[\w*\])'?\"?\s+in\s+\"([\w*,?]*)\"")
...
class Expression(object):
    ...
    def __fixOperators(self, exp):
        exp = re.sub(ur"\seq\s", " = ", exp, 0, re.IGNORECASE)
        exp = re.sub(ur"\s==\s", " = ", exp, 0, re.IGNORECASE)
        exp = re.sub(ur"\s=\*\s", " ILIKE ", exp, 0, re.IGNORECASE)
        exp = re.sub(ur"\sne\s", " != ", exp, 0, re.IGNORECASE)
        exp = re.sub(ur"\slt\s", " < ", exp, 0, re.IGNORECASE)
        exp = re.sub(ur"\sgt\s", " > ", exp, 0, re.IGNORECASE)
        exp = re.sub(ur"\sle\s", " <= ", exp, 0, re.IGNORECASE)
        exp = re.sub(ur"\sge\s", " >= ", exp, 0, re.IGNORECASE)
        #exp = re.sub(ur"\s~\*\s", " ~ ", exp, 0, re.IGNORECASE)
        return exp
