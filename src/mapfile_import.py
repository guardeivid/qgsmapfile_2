...
from qgis.core import (QgsProject, QgsLayerTreeLayer, QgsVectorLayer, \
    QgsRasterLayer, QgsDataSourceURI, QgsMapLayerRegistry)
...
class MapfileImport(object):
    ...
    def parseDataWfs(self, layer):
        ...
        uri = url + urllib.unquote(urllib.urlencode(params))
        return ({"uri": uri}, True)
    ...
    def __addLayer(self, path, mslayer, provider, raster=False):
        ...
        if msgroup:
            addedlayer = QgsMapLayerRegistry.instance().addMapLayer(qgslayer, addToLegend=False)
